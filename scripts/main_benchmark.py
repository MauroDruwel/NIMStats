#!/usr/bin/env python3
"""Main chain benchmark — Ahmed Hassan's 3-layer NIM architecture."""
...
def call_model(model, prompt):
    ...
    try:
        ...
        return {"model": model, "success": False,
                "error": f"Invalid JSON: {exc.msg} at line {exc.lineno} col {exc.colno}",
                "responseTime": response_time, "tokensGenerated": None,
                "totalTokens": None, "response": raw_body}
    ...
    if status_code >= 400:
        error_message = error_message or f"HTTP {status_code} returned by API"
...
def update_history(new_run):
    write_run(new_run, benchmark_type="main")
...
def main():
    ...
    print(f"Starting MAIN chain benchmarks ({MODEL_GROUP or 'all'})...")
